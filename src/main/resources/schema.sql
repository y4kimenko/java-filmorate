...
(
    ...
    CONSTRAINT chk_director_name_not_blank CHECK (TRIM(name) <> '')
);
...
(
    ...
    CONSTRAINT fk_event_feed_user
        FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE,

    CONSTRAINT chk_event_feed_event_type
        CHECK (event_type IN ('LIKE', 'REVIEW', 'FRIEND')),

    CONSTRAINT chk_event_feed_operation
        CHECK (operation IN ('ADD', 'REMOVE', 'UPDATE')),

    CONSTRAINT chk_event_feed_timestamp_non_negative
        CHECK (timestamp >= 0)
);
...
(
    ...
    CONSTRAINT fk_film_directors_film FOREIGN KEY (film_id) REFERENCES film (id) ON DELETE CASCADE,
    CONSTRAINT fk_film_directors_director FOREIGN KEY (director_id) REFERENCES director (id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_event_feed_user_timestamp
    ON event_feed (user_id, timestamp);